...
- `-O[N]`（垂直 split で複数ファイルを開く）
  - 効果: 中〜高
  - コスト: 中
  - 依存:
    - 既存 vsplit UI（済）

- `-p[N]`（tabpage で複数ファイルを開く）
  - 効果: 中〜高
  - コスト: 中
  - 依存:
    - 既存 tabpage（済）
