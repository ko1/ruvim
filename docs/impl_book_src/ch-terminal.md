...
**[代替スクリーンバッファ](#index:代替スクリーンバッファ)** (`?1049h`) は重要な概念だ。端末には主画面と代替画面の 2 つのバッファがある。エディタは代替画面で動作し、終了すると主画面に戻る。つまり、エディタを閉じるとシェルの表示がそのまま復元される。`less` や `vim` と同じ動作だ。
...
端末に「お前の能力を教えろ」とクエリを送り、返ってきた属性リストに `4` が含まれていれば Sixel 対応だ。

> [!NOTE]
> Sixel プロトコルの詳細は [Sixel — ターミナルに画像を描く](ch-sixel.md) で解説している。

この問い合わせ→応答のパターンは `read_terminal_response` で実装されている。
...
> [!IMPORTANT]
> セキュリティ上、外部コマンド実行では必ず引数を配列として渡す。シェル文字列を直接渡すと、ユーザー入力に含まれるメタ文字（`;`, `|`, `$()` など）が解釈されてしまう。
