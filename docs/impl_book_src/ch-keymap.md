...
> [!NOTE]
> このプレフィックスインデックスは、キーの登録時に O(キー長) のコストで構築される。解決時のパフォーマンスがキー数に依存しないのが利点だ。ベンチマークは `benchmark/resolve_layers.rb` で確認できる。
...
> [!TIP]
> この4層の解決は[設定システム](ch-config.md)の `nmap` が filetype-local と global のどちらに登録するかを決める仕組みと対応している。
