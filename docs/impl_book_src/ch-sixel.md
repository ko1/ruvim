...
テキストエディタに画像表示は贅沢に聞こえるが[^1]、Markdown のプレビューや画像ファイルの確認など、実用的な場面は多い。RuVim は **Sixel** プロトコルを使って、ターミナル上に画像を直接描画する。

[^1]: Sixel の対応状況は[ターミナル制御](ch-terminal.md)の DA1 クエリで検出する。
...
> [!TIP]
> `img2sixel` がインストールされている場合は自動的に優先される。`brew install libsixel`（macOS）や `apt install libsixel-bin`（Ubuntu）でインストールできる。
