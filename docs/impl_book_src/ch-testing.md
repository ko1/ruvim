...
[テキストエディタのテスト](#index:テスト戦略)は難しい。ターミナル I/O を使い、ユーザーのキー入力に応答し、画面に出力する。自動テストにはターミナルを模擬する必要がある。
...
> [!TIP]
> この「ターミナルなしで Editor を生成する」アプローチが可能なのは、[依存注入](ch-design-patterns.md)により Editor が Terminal に直接依存していないためだ。
...
> [!NOTE]
> テストは `rake test` で実行できる。テスト失敗時のデバッグには `lumitrace` が有用だ（CLAUDE.md の「Debugging」セクションを参照）。
