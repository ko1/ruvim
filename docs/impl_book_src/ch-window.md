...
左右の移動は、バイト単位でも文字単位でもなく、**[グラフェム・クラスタ](#index:グラフェム・クラスタ)単位** で行う。
...
> [!NOTE]
> Unicode の文字幅計算の詳細は [Unicode 対応 — 文字幅の深淵](ch-unicode.md) で解説している。C 拡張による高速化は [C 拡張](ch-c-extension.md) を参照。
...
水平スクロールでは画面列（表示幅）と文字インデックス（内部位置）の変換が必要になる。タブや全角文字の存在により、これは単純な 1:1 対応ではない。

> [!TIP]
> この座標変換は[画面描画](ch-screen.md)でも重要な役割を果たす。3つの座標系（バッファ、画面、ターミナル）の関係を理解しておくと、描画コードが読みやすくなる。`TextMetrics.screen_col_for_char_index` と `char_index_for_screen_col` がこの変換を担う。
