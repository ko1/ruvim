...
RuVim のアンドゥは **[スナップショット方式](#index:スナップショット方式)** を採用している。変更前の行配列のコピーをスタックに保存する。
...
> [!IMPORTANT]
> この構造共有が安全に動作するのは、Buffer の**すべての変更メソッドが文字列を in-place で変更しない**という不変条件に依存している。`String#<<` や `String#gsub!` を使ってしまうとスナップショットが壊れる。
...
> [!WARNING]
> `Marshal.load` は任意のオブジェクトをデシリアライズするため、信頼できないファイルからの読み込みにはリスクがある。undo ファイルの保存先（`undodir`）はユーザーのホームディレクトリ内に限定されている。
...
> [!NOTE]
> ストリーム連携の詳細は [ストリーム — 非同期 I/O と外部プロセス](ch-streams.md) で解説している。
