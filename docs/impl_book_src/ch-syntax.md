...
すべての[言語モジュール](#index:言語モジュール/Lang::Base)は `Lang::Base` を継承する。
...
> [!NOTE]
> 遅延ロード（autoload）パターンの詳細は[設計パターン](ch-design-patterns.md)を参照。
...
> [!TIP]
> デデントトリガーは最後の1文字だけをチェックするため、`end` と入力する途中（`en` まで）ではトリガーされない。これにより、入力途中のインデント変更による混乱を防いでいる。
