...
[オペレータ](#index:オペレータ)と[モーション](#index:モーション)の組み合わせは、Vim 系エディタの最も強力な武器です。「何をするか」と「どこまでか」を分離することで、少数のコマンドの掛け合わせから無数の操作を生み出せます。一度この文法を体得すれば、新しいモーションを覚えるたびにすべてのオペレータと組み合わせられる — つまり知識が掛け算で増えていきます。

> [!IMPORTANT]
> この章は RuVim の核心です。[基本の移動](ch-basic-movement.md)で学んだコマンドがすべてモーションとして使え、[テキストオブジェクト](ch-text-objects.md)と組み合わせるとさらに強力になります。
...
モーション（どこまで）は[基本の移動](ch-basic-movement.md)で学んだ移動コマンドです。
...
Ruby [filetype](#index:filetype) ではネスト構造に基づく[自動インデント](#index:オートインデント)が適用されます。詳しくは[シンタックスハイライトと言語サポート](ch-syntax-languages.md#オートインデント)を参照してください。
...
> [!NOTE]
> [ヤンク](#index:ヤンク)したテキストが行単位（linewise）の場合、`p` はカーソルの下の行にペーストし、`P` はカーソルの上の行にペーストします。[レジスタ](ch-dot-macro-register.md#レジスタ)を使えば複数のテキストを同時に保持できます。
