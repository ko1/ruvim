...
バージョン管理はソフトウェア開発の基盤です。エディタの中から直接 [Git](#index:Git/Git 連携) を操作できると、「コードを書く → 差分を確認 → コミットする」という流れが途切れません。[blame](#index:Git/git blame) で「誰がなぜこう書いたか」を即座に調べられるのも、コードの理解に大きく貢献します。
...
> [!NOTE]
> コミットメッセージバッファでは `gitcommit` [filetype](ch-syntax-languages.md) が自動設定され、[スペルチェック](ch-misc-editing.md#スペルチェック)がデフォルトで有効になります。
...
Tab [補完](ch-completion.md)で git サブコマンドを補完できます。

> [!CAUTION]
> [Restricted mode](ch-security.md) (`-Z`) では `:git` コマンドは無効化されます。
