...
外部コマンドの出力やログファイルの更新をリアルタイムにエディタ内で確認できるのは、RuVim の大きな強みです。ターミナルマルチプレクサを使わなくても、[`:run`](#index::run) でテストを実行しながら結果を確認したり、[`:follow`](#index::follow) でログを監視したりできます。データの「流れ」をエディタで捉えましょう。
...
> [!TIP]
> [ウィンドウ分割](ch-windows.md)と組み合わせると、コードと実行結果を同時に確認しながら開発できます。
