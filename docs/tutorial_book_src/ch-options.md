...
道具は自分の手に合うように調整してこそ真価を発揮します。RuVim の[オプション](#index:オプション)設定を使いこなせば、行番号の表示、インデント幅、検索の挙動など、自分の作業スタイルに合ったエディタ環境を構築できます。一度設定すれば [init.rb](ch-config.md) に書いて永続化できるので、繰り返し手動で設定する必要もありません。

> [!TIP]
> おすすめの初期設定は[](appendix-g.md)にまとまっています。まずそちらをベースにカスタマイズするのがお勧めです。
