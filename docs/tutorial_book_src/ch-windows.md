...
テストを書きながら実装を見たい。ログを監視しながらコードを書きたい。[ウィンドウ分割](#index:ウィンドウ)を使えば、画面を分割して複数のファイルを同時に見ることができます。1つのターミナルの中で完結するため、tmux や別のターミナルウィンドウを用意する必要がありません。
...
> [!TIP]
> `Shift+矢印キー` でも移動できます。1ウィンドウのときは自動で分割される「スマート分割」が働きます — これは RuVim 独自の機能です。
...
RuVim は[ツリー構造](#index:ネスト分割)のレイアウトをサポートしています[^1]。`:vsplit` 後に `:split` すると、右ウィンドウだけが上下に分割されます。

[^1]: Vim ではウィンドウのネスト構造に制限がありますが、RuVim ではツリー構造で自由にネストできます。詳しくは[Vimとの違い](appendix-e.md)を参照してください。
