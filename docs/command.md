...
- 形式: `:{range}m {address}`
- 行を指定アドレスの後ろに移動する
- `:{range}m$` で末尾に移動、`:{range}m0` で先頭に移動
...
- 形式: `:{range}>`, `:{range}<`
- 行を `shiftwidth` 分だけ右/左にシフトする

### `:normal` / `:norm`

- 形式: `:{range}normal {keys}`
- 各行で Normal mode コマンドを実行する
- 範囲指定時は各行にカーソルを移動して実行
- `:g/pattern/normal A;` のように `:global` と組み合わせて使える
...
- `c`（確認）フラグ: `y/n/a/q/l/Esc` 対応

### `:global` / `:vglobal`

- 形式: `:[range]g/pattern/command`, `:[range]v/pattern/command`
- `g!` は `:vglobal` と同義
- パターンにマッチする（`:v` の場合はマッチしない）全行に対して Ex コマンドを実行
- コマンド省略時は `:p`（print）扱い
- `:global` / `:vglobal` のフルネーム形式にも対応
- undo は一括（一回の `u` で全変更を元に戻せる）
- 行が削除された場合の行番号シフトを自動追跡
