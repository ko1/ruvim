...
構造化データ（TSV/CSV）を見やすく整形して表示する機能です。
...
- **アーキテクチャ**: filetype ごとにレンダラーを登録できる汎用フレームワーク
  ...
  - 現状のレンダラー: `RuVim::RichView::TableRenderer`（TSV/CSV）
- **起動方法**:
  - `:rich [format]` Ex コマンド（トグル）
  - `gr` Normal mode キーバインド（トグル）
- **Rich View バッファ**:
  - `add_virtual_buffer` で作成（kind: `:rich_view`）
  - `readonly = true`, `modifiable = false`, `wrap = false`
  - 元バッファの行をコピーして保持（タブ/カンマ区切りのまま）
  - `:q` で閉じて元バッファに戻る
- **レンダリング**:
  - `Screen` の `plain_window_render_rows` でバッファが Rich View か判定
  - Rich View の場合、表示行だけを `RichView.render_visible_lines` で整形
