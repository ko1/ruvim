...
- 現状の対応 filetype:
  ...
  - `scheme`
  - `diff`
  - `c`（C: キーワード・型名・文字列・数値・コメント・プリプロセッサ・定数マクロ。スマートインデント・保存時 gcc チェック対応）
  - `cpp`（C++: C の全機能 + `class`, `namespace`, `template`, `auto`, `nullptr` 等の C++ キーワード。アクセス指定子のインデント対応・保存時 g++ チェック）
- 優先度（高 -> 低）:
  - cursor / visual
  - search highlight
  - syntax highlight
