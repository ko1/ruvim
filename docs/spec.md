...
- ディレクトリを指定した場合はエラーメッセージを表示し、空バッファを維持する（クラッシュしない）
...
- Ex:
  ...
  - `:setlocal`
  - `:setglobal`
- 実装済み option は `RuVim::Editor::OPTION_DEFS` に定義（現状 `51` 個）
- 代表例:
  - window-local: `number`, `relativenumber`, `wrap`, `linebreak`, `breakindent`, `cursorline`, `scrolloff`, `sidescrolloff`
  - global: `ignorecase`, `smartcase`, `hlsearch`, `incsearch`, `splitbelow`, `splitright`, `hidden`, `clipboard`, `timeoutlen`, `syncload`
...
- **アーキテクチャ**: filetype ごとにレンダラーを登録できる汎用フレームワーク
  ...
  - レンダラー: `TableRenderer`（TSV/CSV）、`MarkdownRenderer`（Markdown）、`JsonRenderer`（JSON）、`JsonlRenderer`（JSONL）、`ImageRenderer`（画像）
  - Markdown `![alt](path)` 画像行: sixel対応ターミナルでは画像を表示、非対応時は `[Image: alt]` プレースホルダーを表示
  - `:set sixel=auto|on|off` で sixel 出力を制御（auto はDA1クエリで自動検出）
  ...
  - `ImageRenderer` は仮想バッファ方式: 画像ファイル（.png/.jpg/.jpeg/.gif/.bmp/.webp）を開くと自動的に `![name](path)` の Markdown 画像行として仮想バッファに表示し、sixel でレンダリング
  - **Sixel DoS 保護**: PNGデコーダで画像サイズ上限（50Mピクセル）、Zlib展開サイズ上限（200MB）、URLダウンロードサイズ上限（10MB）を設定。巨大画像やzip bomb攻撃を防止
