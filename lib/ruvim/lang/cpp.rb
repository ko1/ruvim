  ...
  module Lang
    class Cpp < C
      ...
      def color_columns(text)
        cols = {}
        apply_regex(cols, text, C::CHAR_RE, STRING_COLOR)
        apply_regex(cols, text, C::STRING_RE, STRING_COLOR)
        apply_regex(cols, text, KEYWORD_RE, KEYWORD_COLOR)
        apply_regex(cols, text, C::NUMBER_RE, NUMBER_COLOR)
        apply_regex(cols, text, C::CONSTANT_RE, CONSTANT_COLOR)
        apply_regex(cols, text, C::PREPROCESSOR_RE, "\e[35m")
        apply_regex(cols, text, C::BLOCK_COMMENT_RE, COMMENT_COLOR, override: true)
        apply_regex(cols, text, C::LINE_COMMENT_RE, COMMENT_COLOR, override: true)
        cols
      end
    end
  end
